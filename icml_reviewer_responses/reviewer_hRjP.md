...
The original description of the Oracle-OR lacked clarity. In the revised version, we will provide an algorithmic definition and its interpretation as a recall and classification upper bound. The corresponding algorithm is available here: [oracle_algorithm.pdf](https://github.com/ICMLanonymous2026/MRS_YOLO_ICML26/blob/main/icml_reviewer_responses/oracle_algorithm.pdf).
...
The contribution is best viewed as architectural and representational, but developed and validated in an application-driven setting, which is consistent with the application-oriented track under which the paper is submitted. Although the paper is motivated by RF signal analysis, the underlying idea concerns how to learn from multiple complementary time-frequency representations for detection. In the revised version, we will also add new acoustic results showing that this multi-resolution design transfers beyond RF and remains effective on a real-world dataset: [acoustic_results_figures.md](https://github.com/ICMLanonymous2026/MRS_YOLO_ICML26/tree/main/icml_reviewer_responses/acoustic_results_figures.md).
