...
from detector2026.core.models.mr_yolo_ablation import MRPatchBackboneRTDETRHead  # noqa: E402
...
DEFAULT_RUN_NAME = "mr_patch_backbone_rtdetr_head"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Refine a pre-trained MRPatchBackboneYOLOOne2ManyHead with an RT-DETR head. "
            "Backbone weights are loaded from a checkpoint; the full model is then trained "
            "end-to-end with the Hungarian RT-DETR loss."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Path to a pre-trained MRPatchBackboneYOLOOne2ManyHead best.pt.",
    )
    ...
    # Backbone architecture (must match the pre-trained checkpoint)
    ...
    parser.add_argument("--p3-hw", type=parse_hw, default=(32, 32))
    parser.add_argument("--stride", type=int, default=32)
    # RT-DETR head (ablated configuration)
...
def main() -> None:
    ...
    print("MRPatchBackboneRTDETRHead refinement ablation")
    ...
    print(f"    p3_hw={tuple(args.p3_hw)}  stride={args.stride}")
    print("  RT-DETR head:")
    print(f"    hidden_dim={args.hidden_dim}  num_queries={args.num_queries}")
    ...
    model = MRPatchBackboneRTDETRHead(
