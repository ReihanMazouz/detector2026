...
OUTPUT_JSON = Path(MODEL_SPECS[0]["checkpoint"]).parent / "compare_two_models_vs_oracle.json"
...
def _build_model(spec: Dict[str, Any]) -> YOLOv11:
    model = YOLOv11(
        output_dir=str(Path(spec["checkpoint"]).parent),
    ...
    model.load_weights(str(spec["checkpoint"]), device=DEVICE, eval_mode=True)
    model.eval()
    return model
...
def main() -> None:
    if len(MODEL_SPECS) != 2:
        raise ValueError("Ce script est prevu pour exactement deux modeles dans MODEL_SPECS.")
    if not DATASET_PATH.is_dir():
        raise FileNotFoundError(f"Dataset introuvable: '{DATASET_PATH}'")

    for spec in MODEL_SPECS:
        if not Path(spec["checkpoint"]).is_file():
            raise FileNotFoundError(f"Checkpoint introuvable: '{spec['checkpoint']}'")
    ...
    models = {spec["label"]: _build_model(spec) for spec in MODEL_SPECS}
    aggregate_stats = {spec["label"]: {"tp": [], "fp": [], "fn": []} for spec in MODEL_SPECS}
    aggregate_stats["oracle"] = {"tp": [], "fp": [], "fn": []}
    per_sample = []

    print("[2/4] Comparaison deux modeles vs oracle")
    for sample_path in sample_paths:
        ...
        for spec in MODEL_SPECS:
            label = spec["label"]
            predictions = _run_model_on_sample(models[label], spec, sample_path)
            predictions_by_model[label] = predictions
            prediction_sets.append(predictions)

            sample_stats = _stats_from_predictions(predictions, gt_boxes, gt_labels, gt_snrs, gt_psnrs)
            _merge_stats(aggregate_stats[label], sample_stats)
            sample_counts[label] = _count_stats(sample_stats)
        ...
        _merge_stats(aggregate_stats["oracle"], oracle_sample_stats)
        sample_counts["oracle"] = _count_stats(oracle_sample_stats)
        ...
        per_sample.append(
            {
                ...
                "num_predictions": {
                    spec["label"]: int(len(predictions_by_model[spec["label"]]))
                    for spec in MODEL_SPECS
                },
        ...
        print(
            f"  - {sample_path.stem}: "
            f"{MODEL_SPECS[0]['label']}={sample_counts[MODEL_SPECS[0]['label']]} | "
            f"{MODEL_SPECS[1]['label']}={sample_counts[MODEL_SPECS[1]['label']]} | "
            f"oracle={sample_counts['oracle']}"
        )

    print("[3/4] Calcul des metriques agregees")
    metrics_by_name = {
        name: stats_analysis_with_metrics(
            stats,
            fa=FALSE_ALARM_TARGET,
            to_plot=False,
            class_index_to_name=CLASS_INDEX_TO_NAME,
        )
        for name, stats in aggregate_stats.items()
    }
    ...
    payload = {
        ...
        "aggregated_counts": {
            name: _count_stats(stats) for name, stats in aggregate_stats.items()
        },
    ...
    for name, stats in aggregate_stats.items():
        print(f"{name:12s} counts : {_count_stats(stats)}")

    print("\n[Metriques]")
    metric_names = ("mAP50", "mAP50:95", "avg_recall_low_snr", "avg_recall_medium_snr", "avg_recall_high_snr")
    summaries = {name: _metric_summary(metrics) for name, metrics in metrics_by_name.items()}
    for key in metric_names:
        print(
            f"{key:22s} "
            f"{MODEL_SPECS[0]['label']}={summaries[MODEL_SPECS[0]['label']][key]:.6f} | "
            f"{MODEL_SPECS[1]['label']}={summaries[MODEL_SPECS[1]['label']][key]:.6f} | "
            f"oracle={summaries['oracle'][key]:.6f}"
        )
