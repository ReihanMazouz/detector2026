"""IsotropicRestrictedPatchBackbone refined with an RT-DETR one-to-one head."""
...
from .mr_patch_backbone_yolo_one2many_head import IsotropicRestrictedPatchBackbone
...
class MRPatchBackboneRTDETRHead(BaseModel):
    """IsotropicRestrictedPatchBackbone with a single-scale RT-DETR one-to-one head.

    Designed for refinement: initialise the backbone from a pre-trained
    MRPatchBackboneYOLOOne2ManyHead checkpoint, then train the full model
    end-to-end with the Hungarian-loss RT-DETR head.
    """
    ...
    def __init__(
        ...
        device: str = "cuda:0",
        in_ch: int = 1,
        # ── backbone (mirrors MRPatchBackboneYOLOOne2ManyHead) ──────────────
        d_model: int = 128,
        patch_size: int = 8,
    ...
    def forward(self, inputs: List[torch.Tensor]):
        ...
        return self.detect_one2one(p3, image_size=self._image_hw)

    # ── weight loading ───────────────────────────────────────────────────────

    def load_backbone_weights(self, weights_path: str, device: str = "cpu") -> Tuple[list, list]:
        """Load backbone (and compatible shared) weights from a MRPatchBackboneYOLOOne2ManyHead checkpoint."""
    ...
    def postprocess(self, outputs, conf_thres: float = 0.1, max_det: int = 300, **_):
        ...
        for boxes_i, scores_i, labels_i in zip(boxes, scores, labels):
            ...
            dets = torch.stack((x1, y1, x2, y2, scores_i[keep], labels_i[keep].to(logits.dtype)), dim=1)
    ...
    def fit(
        ...
        **_,
        ...
        if monitor != "val_loss":
            raise ValueError("MRPatchBackboneRTDETRHead only supports monitor='val_loss'.")
